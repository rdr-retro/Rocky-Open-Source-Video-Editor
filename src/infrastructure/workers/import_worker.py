    ...
    def run(self):
        ...
        try:
            ...
            if not is_image:
                ...
                # If FFprobe timed out (w=0, h=0) OR if duration is suspicious (<= 10.0 default usually 5-10s)
                # default initialized duration was 300*1000 ticks (5s).
                is_probe_failed = (w <= 0 or h <= 0 or dur_sec <= 0.001)
                
                if is_probe_failed:
                    print(f"ImportWorker: FFprobe failed or timed out. Falling back to Engine for {os.path.basename(self.file_path)}")
                    try:
                        ...
                        print(f"ImportWorker: Engine Fallback failed: {e}")
