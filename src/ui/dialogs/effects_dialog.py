...
import math
from ..core.models import TICKS_PER_SECOND
...
class EffectsDialog(QDialog):
    ...
    def seek_preview(self, time_s):
        ...
        # Convert time_s (relative to clip start) to absolute ticks
        global_tick = self.clip.start_tick + int(time_s * TICKS_PER_SECOND)
        
        try:
            frame_data = self.rocky_app.engine.evaluate(int(global_tick))
