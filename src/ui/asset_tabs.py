...
class AssetTabsPanel(QFrame):
    ...
    def __init__(self):
        ...
        # 1. Pestaña General (Proyecto)
        self.tab_general = self._create_general_view()
        self._add_png_tab(self.tab_general, "General", "visor.png")

        # 2. Transiciones
        self.tab_transitions = self._create_empty_view()
        self._add_png_tab(self.tab_transitions, "Transiciones", "timeline.png")

        # 3. Efectos
        self.tab_effects = self._create_effects_view()
        self._add_png_tab(self.tab_effects, "Efectos", "efectos.png")

        # 4. Generadores de medios
        self.tab_generators = self._create_empty_view()
        self._add_png_tab(self.tab_generators, "Generadores", "transformador.png")

        # 5. Subtítulos (IA)
        self.tab_subtitle = self._create_empty_view()
        self._add_png_tab(self.tab_subtitle, "Subtítulos", "subtitulos.png")
    ...
    def _add_png_tab(self, widget, label, icon_name):
        """Helper to add professional PNG tabs."""
        ...
        if os.path.exists(icon_path):
            ...
            self.tabs.addTab(widget, QIcon(pix), label)
        else:
