...
class OverlayViewer(QLabel):
    ...
    def _get_thumbnail(self):
        ...
        if hasattr(self.clip, 'thumbnails') and self.clip.thumbnails:
            ...
            print(f"DEBUG: OverlayViewer found {len(self.clip.thumbnails)} thumbnails for {self.clip.name}")
            if len(self.clip.thumbnails) > 1:
                return self.clip.thumbnails[1]
            return self.clip.thumbnails[0]
        else:
            print(f"DEBUG: OverlayViewer: No thumbnails available for {self.clip.name if self.clip else 'Unknown'}")
        return None
