...
class PanelTypeGridMenu(QWidget):
    ...
    def __init__(self, active_type: str = None, parent=None):
        ...
        self.setObjectName("PanelTypeGridMenu")
        # Optimization: Remove system shadow to avoid black artifacts in corners
        ...
        # 1. Main Outer Layout (Margen para sombra)
        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(10, 10, 10, 10)
        
        # 2. The Actual Menu Frame
        ...
        categories = [
            ...
            ("Scripts", [
                ...
                ("texteditor.png", "Editor de texto", "TextEditor", "Shift F11"),
            ]),
            ("Datos", [
                ("propiedades.png", "Propiedades", "Properties", "Shift F7"),
        ...
        for i, (cat_name, items) in enumerate(categories):
            col_widget = QFrame()
            # divisores verticales tipo Blender ultra-oscuros
            is_last = (i == len(categories) - 1)
            border_style = "none" if is_last else "1px solid #222"
            col_widget.setStyleSheet(f"border: none; border-right: {border_style}; background: transparent;")
            col_layout = QVBoxLayout(col_widget)
            col_layout.setContentsMargins(0, 0, 0, 4)
            col_layout.setSpacing(0) # Blender specific: tight vertical list
            ...
            for icon_name, label, p_type, shortcut in items:
            ...
            self.container_layout.addWidget(col_widget)

    def paintEvent(self, event):
        """Force absolute transparency for the root to prevent black artifacts."""
...
class RockyPanelHeader(QFrame):
    ...
    def update_type_icon(self, panel_type):
        """Update the icon based on current panel type using PNG resources."""
        icons = {
            "Viewer": "visor.png",
            "Timeline": "timeline.png",
            "Properties": "propiedades.png",
            "Effects": "efectos.png",
            "MediaTransformer": "transformador.png",
            "MasterMeter": "Master.png",
            "FileBrowser": "folder.png",
...
class RockyPanel(QFrame):
    ...
    def __init__(self, content_widget, title="Editor", parent=None):
        ...
        # Sub-pixel buffer margins to ensure content doesn't touch the border
        # This helps anti-aliasing and prevents "leakage" at the corners
        layout = QVBoxLayout(self)
        layout.setContentsMargins(1, 1, 1, 1) # Space for the 1px border
        layout.setSpacing(0)
        
        # --- VIEWPORT (Definitive Clipping Window) ---
        # This is the "window" the user looks through.
        # It's an internal container that carries the mask.
        self.viewport = QFrame()
        self.viewport.setObjectName("RockyPanelViewport")
        self.viewport.setStyleSheet("background: transparent; border: none;")
        
        # Definitive Clipping logic for the viewport
        def viewport_resize_event(event):
            # The viewport size might be slightly smaller than the panel due to margins
            path = QPainterPath()
            # We use a slightly smaller radius if needed to fit inside the 1px border
            # but usually dt.RADIUS_CONTAINER works best for a tight fit.
            # We subtract a tiny bit to avoid the mask "eating" the border pixels.
            path.addRoundedRect(self.viewport.rect(), dt.RADIUS_CONTAINER - 0.5, dt.RADIUS_CONTAINER - 0.5)
            self.viewport.setMask(QRegion(path.toFillPolygon().toPolygon()))
            
        self.viewport.resizeEvent = viewport_resize_event
        ...
        self.content_area.setStyleSheet(f"""
            ...
            /* Ensure immediately nested widgets also respect the rounding */
            #PanelContentArea > QWidget {{
                border-bottom-left-radius: {dt.RADIUS_CONTAINER - 1}px;
                border-bottom-right-radius: {dt.RADIUS_CONTAINER - 1}px;
            }}
        """)
