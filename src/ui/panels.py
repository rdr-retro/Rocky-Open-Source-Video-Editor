...
class PanelTypeGridMenu(QWidget):
    ...
    def __init__(self, active_type: str = None, parent=None):
        ...
        # 1. Main Outer Layout (Allows space for shadow)
        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(15, 15, 15, 15) # Optimal shadow space
...
class RockyPanelHeader(QFrame):
    ...
    def update_type_icon(self, panel_type):
        ...
        if os.path.exists(icon_path):
            ...
            # Fallback to emoji if PNG not found (higiene)
            fallback = {"Viewer": "📽️", "Timeline": "🎞️"}.get(panel_type, "⚙️")
            self.btn_type.setIcon(QIcon())
            self.btn_type.setText(f"{fallback} ▾")
