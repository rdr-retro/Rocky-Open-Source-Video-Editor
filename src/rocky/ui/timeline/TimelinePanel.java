...
import rocky.core.plugins.*;
import java.awt.datatransfer.*;
import rocky.ui.navigation.PluginTransferHandler;
import rocky.ui.properties.PropertiesWindow;
import rocky.ui.properties.FXChainWindow;
...
public class TimelinePanel extends JPanel {
    ...
    public long getPlayheadFrame() {
        return model.getBlueline().getPlayheadFrame();
    }

    public double getPlayheadTime() {
        return model.getBlueline().getPlayheadFrame() / getFPS();
    }

    public java.util.List<TimelineClip> getClips() {
        return model.getClips();
    }

    public java.util.List<Integer> getTrackHeights() {
        return model.getTrackHeights();
    }
    ...
    public java.util.List<TrackControlPanel.TrackType> getTrackTypes() {
        ...
        if (sidebar != null) {
            for (int i = 0; i < model.getTrackHeights().size(); i++) {
    ...
    public Color getPlayheadColor() {
        return model.getBlueline().getColor();
    }
    ...
    public void updatePlayheadFromFrame(long frame, boolean force) {
        model.getBlueline().setPlayheadFrame(frame);
    ...
    public boolean isPlaying() {
        return model.getBlueline().isPlaying();
    }
    ...
    public void startPlayback() {
        model.getBlueline().startPlayback();
    }

    public void stopPlayback() {
        long start = model.getBlueline().getPlaybackStartFrame();
        model.getBlueline().stopPlayback();
    ...
    public double getPlaybackRate() {
        return model.getBlueline().getPlaybackRate();
    }
    ...
    public TimelinePanel(rocky.core.model.TimelineModel model) {
        ...
        MouseAdapter interactionHandler = new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                for (int i = 0; i < model.getTrackHeights().size(); i++) {
                    int trackH = model.getTrackHeights().get(i);
                    for (TimelineClip clip : model.getClips()) {
            ...
            private void startInteraction(TimelineClip clip, int mode, int x) {
                ...
                activeTrackHeight = model.getTrackHeights().get(clip.getTrackIndex());

                // Remove from tree before modification to ensure key consistency for deletion
                model.getClipTree().remove(originalStart, originalStart + originalDuration, clip);
            }
            ...
            public void mouseDragged(MouseEvent e) {
                if (activeClip != null) {
                    ...
                    if (interactionMode == 1) {
                        ...
                        int trackSum = 0;
                        for (int i = 0; i < model.getTrackHeights().size(); i++) {
                            int h = model.getTrackHeights().get(i);
        ...
        setDropTarget(new java.awt.dnd.DropTarget(this, new java.awt.dnd.DropTargetListener() {
            ...
            public void drop(java.awt.dnd.DropTargetDropEvent dtde) {
                try {
                    ...
                    if (t.isDataFlavorSupported(PluginTransferHandler.PLUGIN_FLAVOR)) {
                        RockyPlugin plugin = (RockyPlugin) t.getTransferData(PluginTransferHandler.PLUGIN_FLAVOR);
                        ...
                        if (targetTrackIdx != -1) {
                            ...
                            if (clip != null) {
                                if (plugin instanceof RockyEffect) {
                                    clip.addEffect(new AppliedPlugin(plugin.getName()));
                                } else if (plugin instanceof RockyTransition) {
                                    long mid = clip.getStartFrame() + (clip.getDurationFrames() / 2);
                                ...
                                TimelineClip gClip = new TimelineClip(plugin.getName(), frame, 150, targetTrackIdx);
                                gClip.setMediaGenerator(new AppliedPlugin(plugin.getName()));
                                addClip(gClip);
                            }
                            fireTimelineUpdated();
                        }
                    ...
                    if (t.isDataFlavorSupported(java.awt.datatransfer.DataFlavor.javaFileListFlavor)) {
                        java.util.List<java.io.File> droppedFiles = (java.util.List<java.io.File>) t
                        ...
                        if (droppedFiles != null && !droppedFiles.isEmpty()) {
                            ...
                            int targetTrackIndex = findTrackAt(loc.y);
                                ...
                                if (mediaPool != null) {
                                    ...
                                    if (source == null) {
                                        source = new rocky.core.media.MediaSource(mediaId, file.getAbsolutePath(),
                                                projectProps.getPreviewScale(), projectProps.getFPS(),
                                                projectProps.getAudioSampleRate());
    ...
    private TimelineClip findClipAt(int trackIndex, long frame) {
        for (TimelineClip clip : model.getClips()) {
            if (clip.getTrackIndex() == trackIndex && frame >= clip.getStartFrame()
                    && frame < clip.getStartFrame() + clip.getDurationFrames()) {
                return clip;
            }
        }
        return null;
    ...
    public void setTrackHeights(java.util.List<Integer> heights) {
        model.setTrackHeights(heights);
        revalidate();
        repaint();
    }

    public void setTrackTypes(java.util.List<TrackControlPanel.TrackType> types) {
        model.setTrackTypes(types);
    }

    @Override
    public Dimension getPreferredSize() {
        int totalHeight = 0;
        for (Integer trackH : model.getTrackHeights()) {
            totalHeight += trackH;
        }

        int minHeight = getParent() != null ? getParent().getHeight() : 600;
        return new Dimension(getWidth(), Math.max(minHeight, totalHeight));
    }
    ...
    private long findSnapFrame(long targetFrame, TimelineClip excludeClip) {
        ...
        long ph = model.getBlueline().getPlayheadFrame();
    ...
    public String formatTimecode(long totalFrames) {
        return model.getBlueline().formatTimecode(totalFrames);
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        java.util.List<TimelineClip> snapshot = model.getClips();
        for (TimelineClip clip : snapshot) {
            if (clip.getTrackIndex() < trackYPositions.size()) {
                ...
                int trackH = model.getTrackHeights().get(clip.getTrackIndex());
        ...
        int playheadX = timeToScreen(model.getBlueline().getPlayheadFrame() / (double) getFPS());
        if (playheadX >= -20 && playheadX <= width + 20) {
            g2d.setColor(model.getBlueline().getColor());
    ...
    /**
     * Zooms out to fit the entire project content in the timeline view with a smooth animation.
     */
    public void zoomToFit() {
        if (getWidth() <= 0) return;
        
        long maxFrame = model.getMaxFrame();
        if (maxFrame == 0) return;
        
        double duration = maxFrame / (double) getFPS();
        // Target: project fits in 45% of the screen width (extreme margin for long-term planning)
        double targetPPS = (getWidth() * 0.45) / duration;
        
        // Clamp to editor limits
        targetPPS = Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, targetPPS));
        
        double startPPS = this.pixelsPerSecond;
        double startScroll = this.visibleStartTime;
        double targetScroll = 0;

        if (zoomAnimator != null && zoomAnimator.isRunning()) zoomAnimator.stop();
        
        final double fTargetPPS = targetPPS;
        final double fStartPPS = startPPS;
        final double fStartScroll = startScroll;
        final long startTime = System.currentTimeMillis();
        final long animDuration = 600; // ms

        zoomAnimator = new Timer(15, e -> {
            long elapsed = System.currentTimeMillis() - startTime;
            double progress = Math.min(1.0, elapsed / (double) animDuration);
            
            // Cubic Out Easing: f(t) = 1 - (1-t)^3
            double ease = 1 - Math.pow(1 - progress, 3);
            
            this.pixelsPerSecond = fStartPPS + (fTargetPPS - fStartPPS) * ease;
            this.visibleStartTime = fStartScroll + (targetScroll - fStartScroll) * ease;
    ...
    private void onModelUpdated() {
        // Propagate model change to external listeners (MainAB -> FrameServer)
