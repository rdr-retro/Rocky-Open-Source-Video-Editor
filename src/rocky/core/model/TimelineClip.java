...
public class TimelineClip {
    ...
    public AppliedPlugin getMediaGenerator() {
        return mediaGenerator;
    }

    public void setMediaGenerator(AppliedPlugin g) {
        this.mediaGenerator = g;
    }

    public AppliedPlugin getFadeInTransition() {
        return fadeInTransition;
    }

    public void setFadeInTransition(AppliedPlugin t) {
        this.fadeInTransition = t;
    }

    public AppliedPlugin getFadeOutTransition() {
        return fadeOutTransition;
    }

    public void setFadeOutTransition(AppliedPlugin t) {
        this.fadeOutTransition = t;
    }

    public List<AppliedPlugin> getAppliedEffects() {
    ...
    public void addKeyframe(TimelineKeyframe k) {
        synchronized (timeKeyframes) {
            // Remove any existing keyframe at the same frame to prevent duplicates
            timeKeyframes.removeIf(existing -> existing.getClipFrame() == k.getClipFrame());
            timeKeyframes.add(k);
            sortKeyframes();
        }
    }
    ...
    public TimelineClip copy() {
        ...
        clone.mediaGenerator = (this.mediaGenerator != null) ? new AppliedPlugin(this.mediaGenerator.getPluginName())
                : null;
        if (clone.mediaGenerator != null)
            clone.mediaGenerator.getParameters().putAll(this.mediaGenerator.getParameters());
        clone.fadeInTransition = (this.fadeInTransition != null)
                ? new AppliedPlugin(this.fadeInTransition.getPluginName())
                : null;
        if (clone.fadeInTransition != null)
            clone.fadeInTransition.getParameters().putAll(this.fadeInTransition.getParameters());
        clone.fadeOutTransition = (this.fadeOutTransition != null)
                ? new AppliedPlugin(this.fadeOutTransition.getPluginName())
                : null;
        if (clone.fadeOutTransition != null)
            clone.fadeOutTransition.getParameters().putAll(this.fadeOutTransition.getParameters());
