    ...
    public void render(File outputFile, RenderProgressListener listener) {
        new Thread(() -> {
            ...
            try {
                ...
                String videoCodec = rocky.core.os.Platform.getHardwareEncoder();
                boolean useBitrate = rocky.core.os.Platform.supportsHardwareBitrate();
                
                String bitrate = "8000k"; // Default 8Mbps
                
                // Adjust bitrate for 4K/1080p
                if (width >= 3840) bitrate = "40000k"; // 40 Mbps for 4K
                else if (width >= 1920) bitrate = "12000k"; // 12 Mbps for 1080p
                
                java.util.List<String> cmd = new java.util.ArrayList<>();
                cmd.add("ffmpeg"); cmd.add("-y");
                cmd.add("-f"); cmd.add("rawvideo");
                cmd.add("-pixel_format"); cmd.add("bgra");
                cmd.add("-video_size"); cmd.add(width + "x" + height);
                cmd.add("-r"); cmd.add(String.valueOf(fps));
                cmd.add("-i"); cmd.add("-");
                cmd.add("-i"); cmd.add(tempAudioFile.getAbsolutePath());
                
                cmd.add("-c:v"); cmd.add(videoCodec);
                
                if (useBitrate) {
                   // VideoToolbox/Hardware encoders often need direct bitrate control
                   cmd.add("-b:v"); cmd.add(bitrate);
                   // Increase ease of playback compatibility
                   cmd.add("-profile:v"); cmd.add("high"); 
                } else {
                   // Generic x264
                   cmd.add("-pix_fmt"); cmd.add("yuv420p");
                   cmd.add("-crf"); cmd.add("18"); // Constant quality
                }
                ...
                // Use Buffered Stream to reduce syscall overhead
                OutputStream os = new java.io.BufferedOutputStream(process.getOutputStream(), 65536);
