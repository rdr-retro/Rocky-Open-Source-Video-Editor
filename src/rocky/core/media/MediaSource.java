...
public class MediaSource {
    ...
    public MediaSource(String id, String filePath, double previewScale, double fps, int sampleRate) {
        this.id = id;
        ...
        if (isVideo) {
            ...
            this.videoDecoder.setScaleFactor(previewScale);
            this.videoDecoder.setVideoFPS(fps);
        ...
        if (isVideo || isAudio) {
            this.audioDecoder = new AudioDecoder(new File(filePath), fps, sampleRate);
            if (this.audioDecoder.init() && this.audioDecoder.hasAudio()) {
                if (isAudio) {
                    this.totalFrames = this.audioDecoder.getTotalFrames();
                }
                System.out.println("[MediaSource] Audio probe SUCCESS: " + filePath + " (FPS: " + fps + " SR: " + sampleRate + ")");
            } else {
    ...
    public void setProjectSettings(double fps, int sampleRate) {
        if (audioDecoder != null) {
            audioDecoder.setProjectSettings(fps, sampleRate);
        }
        if (videoDecoder != null) {
            videoDecoder.setVideoFPS(fps);
        }
        if (proxyDecoder != null) {
            proxyDecoder.setVideoFPS(fps);
        }
    }
    ...
    public void reinitDecoder(double scale) {
        ...
        // We need a way to get the project properties here, or assume it's set later.
        // Actually, reinitDecoder is called from MainAB where we can pass it if needed.
        // But setProjectSettings is also called right after.
        if (this.videoDecoder.init()) {
            this.width = this.videoDecoder.getWidth();
            this.height = this.videoDecoder.getHeight();
        }
    ...
    /**
     * Explicitly releases native resources associated with this source.
     * Should be called when the project is closed or the clip is removed permanently.
     */
    public void dispose() {
        if (videoDecoder != null) {
            videoDecoder.close();
