...
public class FrameServer {
    ...
    private void drawAssetOnCanvas(java.awt.Graphics2D g2, BufferedImage asset, TimelineClip clip, long frameInClip,
        ...
        if (properties.isPluginsEnabled() && !clip.getAppliedEffects().isEmpty()) {
            ...
            // We clone the asset to avoid modifying the original buffered frame (which might be in cache)
            BufferedImage effectBuffer = new BufferedImage(assetW, assetH, BufferedImage.TYPE_INT_ARGB_PRE);
            Graphics2D eg2 = effectBuffer.createGraphics();
            ...
            synchronized(clip.getAppliedEffects()) {
                for (AppliedPlugin applied : clip.getAppliedEffects()) {
                    ...
                    if (effect != null) {
                        try {
                            effect.apply(effectBuffer, eg2, applied.getParameters());
                        } catch (Exception e) {
