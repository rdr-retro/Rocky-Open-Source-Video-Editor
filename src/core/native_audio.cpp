...
std::vector<float> NativeAudioSource::getPeaks(double startTime, double reqDuration, int numBuckets) {
    ...
    std::vector<float> peaks(numBuckets * 2, 0.0f); // Store min, max pairs
    
    long startSample = (long)(startTime * sample_rate);
    long durationSamples = (long)(reqDuration * sample_rate);
    
    if (durationSamples <= 0) return peaks;

    double samples_per_bucket = (double)durationSamples / numBuckets;

    for (int i = 0; i < numBuckets; ++i) {
        long bucketStart = startSample + (long)(i * samples_per_bucket);
        long bucketEnd = startSample + (long)((i + 1) * samples_per_bucket);
        
        // Clamp
        bucketStart = std::max(0L, std::min((long)total_samples - 1, bucketStart));
        bucketEnd = std::max(0L, std::min((long)total_samples, bucketEnd));
        ...
        for (long s = bucketStart; s < bucketEnd; ++s) {
            // Check both channels (Left and Right) if available
