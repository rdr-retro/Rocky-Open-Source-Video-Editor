...
VideoSource::VideoSource(std::string p) : path(p) {
  {
    ...
    av_dict_set(&opts, "probesize", "5000000",
                0); // 5MB max para detectar formato
    av_dict_set(&opts, "analyzeduration", "1000000",
                0);                             // 1s max para detectar streams
    ...
    if (avformat_find_stream_info(fmt_ctx, nullptr) < 0) {
      std::cerr << "[VideoSource] Failed to find stream info: " << path
                << std::endl;
      is_valid = false;
      av_dict_free(&opts);
      return;
  ...
  if (video_stream_idx != -1) {
    const AVCodec *v_codec = avcodec_find_decoder(
...
int VideoSource::getRotation() const {
  ...
  AVDictionaryEntry *tag = av_dict_get(
      fmt_ctx->streams[video_stream_idx]->metadata, "rotate", NULL, 0);
  if (tag)
    return std::atoi(tag->value);
  return 0;
}

int VideoSource::getNativeWidth() const {
  if (codec_ctx && codec_ctx->width > 0)
    return codec_ctx->width;
  if (video_stream_idx != -1 &&
      fmt_ctx->streams[video_stream_idx]->codecpar->width > 0)
    return fmt_ctx->streams[video_stream_idx]->codecpar->width;
  return -1;
}

int VideoSource::getNativeHeight() const {
  if (codec_ctx && codec_ctx->height > 0)
    return codec_ctx->height;
  if (video_stream_idx != -1 &&
      fmt_ctx->streams[video_stream_idx]->codecpar->height > 0)
    return fmt_ctx->streams[video_stream_idx]->codecpar->height;
  return -1;
}

int VideoSource::getWidth() const { return getNativeWidth(); }

int VideoSource::getHeight() const { return getNativeHeight(); }
...
Frame VideoSource::getFrame(double localTime, int w, int h) {
  ...
  if (last_frame && std::abs(localTime - last_time) < 0.001 && w == last_w &&
      h == last_h) {
    return *last_frame;
  }
  ...
  const int64_t targetPts =
      static_cast<int64_t>(localTime / av_q2d(timeBase) + 0.001);
      ...
      if (avcodec_send_packet(codec_ctx, pkt) >= 0) {
        while (avcodec_receive_frame(codec_ctx, av_frame) >= 0) {
          if (av_frame->pts >= targetPts) {
            ...
            // SIMPLIFIED ENGINE: Just return the raw frame scaled to (nativeW,
            // nativeH) Fitting and rotation is now handled by the Clip layer
            // for consistency.

            SwsContext *new_ctx = sws_getCachedContext(
                sws_ctx, av_frame->width, av_frame->height,
                static_cast<AVPixelFormat>(av_frame->format), w, h,
            ...
            uint8_t *dstPointers[4] = {outputFrame.data.data(), nullptr,
                                       nullptr, nullptr};
            int dstStrides[4] = {w * 4, 0, 0, 0};
            sws_scale(sws_ctx, av_frame->data, av_frame->linesize, 0,
                      av_frame->height, dstPointers, dstStrides);
...
std::vector<float> VideoSource::getAudioSamples(double startTime,
  ...
  const size_t target_sample_count =
      static_cast<size_t>(duration * target_sample_rate);
  ...
  if (!cached_swr) {
    ...
    swr_alloc_set_opts2(&cached_swr, &out_ch_layout, AV_SAMPLE_FMT_FLT,
                        target_sample_rate, &audio_codec_ctx->ch_layout,
      ...
      if (avcodec_send_packet(audio_codec_ctx, audio_pkt) >= 0) {
        while (avcodec_receive_frame(audio_codec_ctx, audio_frame) >= 0) {
          ...
          if (frameEnd > startTime) {
            int out_samples =
                swr_get_out_samples(cached_swr, audio_frame->nb_samples);
            std::vector<float> resampledBuffer(out_samples * target_channels);
            float *out_data[1] = {resampledBuffer.data()};
            swr_convert(cached_swr, (uint8_t **)out_data, out_samples,
            ...
            if (copyEnd > copyStart) {
              size_t copyOffset = static_cast<size_t>((copyStart - frameStart) *
                                                      target_sample_rate) *
                                  target_channels;
              size_t availToCopy = static_cast<size_t>((copyEnd - copyStart) *
                                                       target_sample_rate) *
                                   target_channels;
              if (samples.size() + availToCopy <=
                  target_sample_count * target_channels) {
                samples.insert(
                    samples.end(), resampledBuffer.begin() + copyOffset,
                    resampledBuffer.begin() + copyOffset + availToCopy);
              }
            }
          }
  ...
  if (samples.size() < target_sample_count * target_channels)
    samples.resize(target_sample_count * target_channels, 0.0f);
  return samples;
}
std::vector<float> VideoSource::getPeaks(double startTime, double duration, int numBuckets) {
  if (numBuckets <= 0) return {};
  
  // Reuse getAudioSamples logic for simplicity, though less efficient than a direct scan
  std::vector<float> samples = getAudioSamples(startTime, duration);
  if (samples.empty()) return std::vector<float>(numBuckets * 2, 0.0f);

  std::vector<float> peaks(numBuckets * 2, 0.0f);
  int target_channels = 2;
  size_t samples_per_bucket = (samples.size() / target_channels) / numBuckets;

  for (int i = 0; i < numBuckets; ++i) {
    size_t start = i * samples_per_bucket * target_channels;
    size_t end = std::min((i + 1) * samples_per_bucket * target_channels, samples.size());
...
void ImageSource::load(int w, int h) {
  ...
  if (avformat_find_stream_info(fmt, nullptr) < 0) {
    std::cerr << "[ImageSource] Failed to find stream info: " << path
              << std::endl;
  ...
  if (stream_idx == -1) {
    std::cerr << "[ImageSource] No video stream found in image file: " << path
              << std::endl;
    avformat_close_input(&fmt);
    cached_frame = Frame(w, h);
    is_valid = false;
  ...
  const AVCodec *codec =
      avcodec_find_decoder(fmt->streams[stream_idx]->codecpar->codec_id);
  if (!codec) {
    std::cerr << "[ImageSource] Codec not found for image: " << path
              << std::endl;
    avformat_close_input(&fmt);
    cached_frame = Frame(w, h);
    is_valid = false;
  ...
  if (avcodec_open2(ctx, codec, nullptr) < 0) {
    std::cerr << "[ImageSource] Failed to open codec for image: " << path
              << std::endl;
    ...
    if (pkt->stream_index == stream_idx) {
      if (avcodec_send_packet(ctx, pkt) >= 0) {
        if (avcodec_receive_frame(ctx, frame) >= 0) {
          ...
          if (sws) {
            uint8_t *dest[4] = {out.data.data(), nullptr, nullptr, nullptr};
            int strides[4] = {w * 4, 0, 0, 0};
            sws_scale(sws, frame->data, frame->linesize, 0, frame->height, dest,
                      strides);
            sws_freeContext(sws);
            cached_frame = out;
            frame_received = true;
  ...
  if (!frame_received) {
    std::cerr << "[ImageSource] Failed to receive frame from image: " << path
              << std::endl;
    is_valid = false;
  } else {
    loaded = true;
