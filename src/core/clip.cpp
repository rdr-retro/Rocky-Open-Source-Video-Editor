...
Frame Clip::render(double time, int w, int h, double fps, long absoluteFrame) {
    ...
    for (int y = startY; y < endY; ++y) {
        for (int x = startX; x < endX; ++x) {
            // Inverse Transform:
            // 1. Translate pixel to relative calculation space (relative to Dest Center)
            double bX = x - dstCX;
            double bY = y - dstCY;
            
            // 2. Inverse Rotate (Rotate by -theta)
            // rotX = x*cos + y*sin
            // rotY = -x*sin + y*cos
            double rX = bX * cos_t + bY * sin_t;
            double rY = -bX * sin_t + bY * cos_t;
            
            // 3. Inverse Scale
            double sX = rX / sx;
            double sY = rY / sy;
            
            // 4. Translate back to Source Coordinate Space (Top-Left 0,0)
            // Add Source Center
            double u = sX + srcCX;
            double v = sY + srcCY;
            ...
            // 5. Boundary Check & Sample
            if (iU >= 0 && iU <= srcMaxX && iV >= 0 && iV <= srcMaxY) {
                // dst[y][x] = src[v][u]
                dstData[y * w + x] = srcData[iV * f.width + iU];
            }
        }
    }
