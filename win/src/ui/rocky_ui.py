...
class RenderWorker(QThread):
    ...
    def run(self):
        # 0. Determin ffmpeg executable path robustly
        ffmpeg_exe = "ffmpeg"
        if sys.platform == "win32":
            ...
            bundled_ffmpeg = os.path.join(base_path, "external", "ffmpeg", "bin", "ffmpeg.exe")
            if os.path.exists(bundled_ffmpeg):
                ffmpeg_exe = bundled_ffmpeg
                print(f"DEBUG: Using bundled FFmpeg: {ffmpeg_exe}", flush=True)

        audio_temp_path = self.output_path + ".audio.tmp"
        
        try:
            # 1. Ensure dimensions are even (FFmpeg requirement for most codecs)
